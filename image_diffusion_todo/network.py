    ...
    def forward(self, x, timestep, class_label=None):
        ...
        if self.use_cfg and class_label is not None:
            if self.training:
                ...
                mask = torch.bernoulli(torch.ones_like(class_label) * (1 - self.cfg_dropout)).type(class_label.dtype)
            ...
            class_label = class_label.to(timestep.device)
            cemb = self.class_embedding(class_label)
            if not timestep.size():
                temb = temb.repeat(len(class_label), 1)
            temb += cemb
            #######################
